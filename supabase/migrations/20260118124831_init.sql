...
-- Drop first to allow re-running (idempotency)
...
drop policy if exists "Users can view their own plans." on generated_plans;
create policy "Users can view their own plans." on generated_plans
  for select using (auth.uid() = user_id);

drop policy if exists "Users can insert their own plans." on generated_plans;
