...
export default function Home() {
  ...
  const jsonLd = {
    ...
    "description": "Generate hyper-personalized diet and workout plans tailored to your unique biology using advanced AI.",
    "offers": {
      "@type": "Offer",
      "price": "0",
      "priceCurrency": "USD",
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-black selection:bg-emerald-500/30 selection:text-emerald-500">
      ...
      <main>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            <AnimatedSection>
              ...
              <p className="mt-8 text-xl md:text-2xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto mb-12 leading-relaxed font-light">
                Stop guessing. Start thriving. Get a hyper-personalized nutrition and workout plan tailored to your unique biology, completely powered by advanced AI.
              </p>
              
              <div className="flex flex-col sm:flex-row justify-center items-center gap-5">
                <Link href={planLink}>
                  <Button size="lg" className="group text-lg px-10 py-7 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 shadow-xl shadow-slate-900/10 dark:shadow-white/5 transition-all duration-300 hover:-translate-y-1">
                    {user ? "Go to Dashboard" : "Get Your Custom Plan"}
                    <ArrowRight className="ml-2 h-5 w-5 transition-transform group-hover:translate-x-1" />
                  </Button>
                </Link>
                <Link href="#how-it-works">
                  <Button variant="outline" size="lg" className="text-lg px-10 py-7 rounded-full border-2 border-slate-200 dark:border-slate-800 bg-transparent hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-300">
                    See How It Works
                  </Button>
                </Link>
              </div>
              ...
              <div className="mt-16 pt-10 flex flex-col items-center">
                 <p className="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-6">Backed by Data, Powered by Intelligence</p>
                 <div className="flex flex-wrap justify-center gap-x-8 gap-y-4 text-slate-500 dark:text-slate-400">
                    <div className="flex items-center space-x-2"><ShieldCheck className="w-5 h-5 text-emerald-500"/> <span className="font-medium">100% Private Data</span></div>
                    <div className="flex items-center space-x-2"><Zap className="w-5 h-5 text-amber-500"/> <span className="font-medium">Instant Generation</span></div>
                    <div className="flex items-center space-x-2"><Users className="w-5 h-5 text-blue-500"/> <span className="font-medium">Expert Vetted</span></div>
                 </div>
              </div>
            </AnimatedSection>
          </div>
        ...
        <section id="features" className="py-32 bg-white dark:bg-slate-950 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <AnimatedSection className="text-center mb-24">
              <h2 className="text-sm font-bold text-emerald-600 dark:text-emerald-400 tracking-wider uppercase mb-3">Why WellNourish AI?</h2>
        ...
        <section id="how-it-works" className="py-32 bg-slate-50 dark:bg-slate-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <AnimatedSection className="mb-20 text-center">
              <h2 className="text-4xl font-bold text-slate-900 dark:text-white mb-4">How it works</h2>
              <p className="text-xl text-slate-600 dark:text-slate-400">Three simple steps to your new lifestyle.</p>
            </AnimatedSection>
            ...
            <AnimatedSection>
              <div className="group relative rounded-[3rem] overflow-hidden bg-emerald-900 shadow-2xl">
                ...
                <div className="relative z-10 px-8 py-20 md:p-24 text-center">
                  ...
                  <Link href={planLink}>
                    <Button size="lg" className="bg-white text-emerald-900 hover:bg-emerald-50 border-none px-12 py-8 text-xl rounded-full font-bold shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
                      Generate My Plan Now
                    </Button>
                  </Link>
                  <p className="mt-6 text-sm text-emerald-200/60">No credit card required for initial generation.</p>
                </div>
              </div>
            </AnimatedSection>
          </div>
...
function FeatureCard({ icon, title, description, delay, color }: { icon: React.ReactNode, title: string, description: string, delay: number, color: string }) {
  return (
    <AnimatedSection delay={delay} className="group p-10 bg-white dark:bg-slate-900 rounded-[2rem] hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-slate-100 dark:border-slate-800 relative overflow-hidden">
      <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${color} opacity-5 rounded-bl-[100px] transition-opacity group-hover:opacity-10`}></div>
      
      <div className={`mb-8 bg-gradient-to-br ${color} p-4 rounded-2xl w-fit shadow-lg group-hover:scale-110 transition-transform duration-500`}>
        {icon}
      </div>
...
function Step({ number, title, description, side }: { number: string, title: string, description: string, side: "left" | "right" }) {
  return (
    <div className={`flex flex-col md:flex-row items-center justify-between mb-16 md:mb-0 relative ${side === 'right' ? 'md:flex-row-reverse' : ''}`}>
       ...
       <div className={`w-full md:w-[45%] ${side === 'left' ? 'md:text-right md:pr-12' : 'md:text-left md:pl-12'} mb-8 md:mb-0`}>
         <AnimatedSection>
            <div className={`inline-block p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 mb-4 md:hidden`}>
       ...
       <div className={`w-full md:w-[45%] ${side === 'left' ? 'md:pl-12' : 'md:pr-12'}`}>
         {/* Placeholder for visual or icon for the step */}
         <div className={`h-64 rounded-3xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 flex items-center justify-center text-slate-300 dark:text-slate-600 text-6xl font-black opacity-50`}>
           {number}
         </div>
       </div>
    </div>
...
function FAQItem({ question, answer }: { question: string, answer: string }) {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900 overflow-hidden">
      ...
      >
        <span className="font-bold text-lg text-slate-900 dark:text-white">{question}</span>
